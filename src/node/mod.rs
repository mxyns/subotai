...
pub mod receptions;
pub use crate::node::factory::Factory;
    ...
    V: Storable + 'static,
{
        ...
        thread::spawn(move || {
            for _ in 0..BOOTSTRAP_TRIES {
                if bootstrap_resources
                    .probe(
                        &bootstrap_resources.id,
                        bootstrap_resources.configuration.k_factor,
                    )
                    .is_ok()
                {
                    break;
    ...
    fn with_configuration(
        ...
        resources
            .inbound
            .set_read_timeout(Some(StdDuration::from_millis(SOCKET_TIMEOUT_MS)))?;
    ...
    fn reception_loop(resources: sync::Arc<resources::Resources<V>>) {
        ...
        loop {
            ...
            resources
                ...
                .lock()
                .unwrap()
                .broadcast(resources::ReceptionUpdate::Tick);
        }
    }
    ...
    fn republish_loop(resources: sync::Arc<resources::Resources<V>>) {
        ...
        for update in updates {
            match update {
                resources::NetworkUpdate::StateChange(State::ShuttingDown) => {
                    break;
                }
    ...
    fn maintenance_loop(resources: sync::Arc<resources::Resources<V>>) {
        ...
        loop {
            ...
            match resources.table.oldest_bucket() {
                (i, None) => {
                    resources.refresh_bucket(i);
                }
                (i, Some(time)) if (now - time) > hour => {
                    resources.refresh_bucket(i);
                }
                _ => (),
            }
    ...
    fn conflict_resolution_loop(resources: sync::Arc<resources::Resources<V>>) {
        loop {
            let conflicts_empty = {
                ...
                conflicts
                    .retain(|&routing::EvictionConflict { times_pinged, .. }| times_pinged < 5);
