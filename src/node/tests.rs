...
use crate::{hash, node, routing, storage};
...
fn test_one_to_n_storage() {
    ...
    origin
        .store(key.clone(), StorageEntry::Blob(vec![0, 1, 2, 3]))
        .expect("could not store value 0");
    origin
        .store(key.clone(), StorageEntry::Blob(vec![3, 2, 1, 0]))
        .expect("could not store value 1");
    origin
        .store(key.clone(), StorageEntry::Blob(vec![0, 2, 1, 3]))
        .expect("could not store value 2");
...
fn bootstrapping_and_finding_on_simulated_network() {
    ...
    assert_eq!(
        head.resources.locate(tail.id()).unwrap().id,
        tail.resources.local_info().id
    );
}
...
fn simulated_network(network_size: usize) -> VecDeque<node::Node> {
        ...
        .collect();
    {
        ...
        for node in nodes.iter().skip(1) {
            node.bootstrap(&origin.resources.local_info().address)
                .unwrap();
        }
...
fn generating_a_conflict_causes_a_ping_to_the_evicted_node() {
    ...
    alpha
        .resources
        .table
        .fill_bucket(index, (cfg.k_factor - 1) as u8);
...
fn node_probing_in_simulated_network() {
    ...
    let mut info_nodes: Vec<routing::NodeInfo> = nodes
        .iter()
        .map(|node| node.resources.local_info())
        .collect();
    ...
    info_nodes.sort_by(|info_a, info_b| (&info_a.id ^ tail.id()).cmp(&(&info_b.id ^ tail.id())));
...
fn store_retrieve_in_simulated_network() {
    ...
    let collection: Vec<_> = (0..10)
        .map(|_| {
            (
                storage::StorageEntry::Value(hash::SubotaiHash::random()),
    ...
    head.resources
        .mass_store(collection_key.clone(), collection.clone())
        .unwrap();
